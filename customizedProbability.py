...
#vocab includes all substrings of lengths 1 - 3 of for a,b,...,z,aa,..,zz,aaa,..,zzz
#including the number  symbol # and punctuation hyphen and apostrophe;
#the final 1 added accounts for the empty string epsilon
#|V| = 27,931
...
class DictionaryProbDist( ProbDistI ):
    ...
    def __init__( self, prob_dict = None, log = False, normalize = False, **kwargs ):
        """
...
class MLEProbDist( ProbDistI ):
    ...
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
...
class LidstoneProbDist( ProbDistI ):
    ...
    *gamma* to the count for each bin, and taking the maximum
    ...
    def __init__( self, freqdist, gamma, bins = None, **kwargs ):
        """
...
class Lidstone_01( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 0.1, bins )
...
class Lidstone_03( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        Use the expected likelihood estimate to create a probability
        distribution for the experiment used to generate ``freqdist``.
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 0.3, bins )
...
class Lidstone_07( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        Use the expected likelihood estimate to create a probability
        distribution for the experiment used to generate ``freqdist``.
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 0.7, bins )
...
class Lidstone_005( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        Use the expected likelihood estimate to create a probability
        distribution for the experiment used to generate ``freqdist``.
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 0.05, bins )
...
class Lidstone_15( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        Use the expected likelihood estimate to create a probability
        distribution for the experiment used to generate ``freqdist``.
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 1.5, bins )
...
class Lidstone_003( LidstoneProbDist ):
    def __init__( self, freqdist, bins = None, **kwargs ):
        """
        Use the expected likelihood estimate to create a probability
        distribution for the experiment used to generate ``freqdist``.
        ...
        :param bins: The number of sample values that can be generated
            ...
            ``bins`` is not specified, it defaults to ``freqdist.B()``.
        """
        LidstoneProbDist.__init__( self, freqdist, 0.03, bins )
...
class CrossValidationProbDist( ProbDistI ):
    ...
    def __init__( self, freqdists, bins, **kwargs ):
        """
...
_MAPCOUNTS = 0
_FERTDICT = 1
# with open( 'PickledData/fertilityData.pickle', 'rb' ) as pklfile1,\
#     open('PickledData/KNcomponents.pickle','rb') as pklfile2:
#     _fertilityData = pickle.load( pklfile1 )
#     _KNcomponents=pickle.load(pklfile2)

class modifiedKneserNey( ProbDistI ):
    def __init__( self, fredist, bins = None, **kwargs ):
        self._freqdist = fredist
        self._bins = bins
        self._totalMapTypes = _fertilityData[ _MAPCOUNTS ]
        self._state = kwargs.get( 'state' )
        self._fertility = _fertilityData[ _FERTDICT ][ self._state ]
        self._N1 = _KNcomponents[ 0 ]
        self._N2 = _KNcomponents[ 1 ]
        self._N3 = _KNcomponents[ 2 ]
        self._N4 = _KNcomponents[ 3 ]

        self._gamma = self.D( 1 ) * self._freqdist.Nr( r = 1 ) + self.D( 2 ) * self._freqdist.Nr( r = 2 ) + \
                      self.D( 3 ) * self._freqdist.N( ) - (self._freqdist.Nr( r = 1 ) + self._freqdist.Nr( r = 2 ))
    ...
    def prob( self, sample ):
        ...
        try:
            p = ((count - self.D( count )) / self._freqdist.N( )) + \
                self._gamma * self._fertility[ sample ] / self._totalMapTypes
    ...
    def D( self, count ):
        ...
        Y = self._N1 / (self._N1 + 2 * self._N2)

        if not count:
            return 0
        elif count == 1:
            return 1 - 2 * Y * self._N2 / self._N1
        elif count == 2:
            return 2 - 3 * Y * self._N3 / self._N2
        elif count >= 3:
            return 3 - 4 * Y * self._N4 / self._N3
...
@compat.python_2_unicode_compatible
class modifiedWittenBellProbDist( ProbDistI ):
    """
    The Witten-Bell estimate of a probability distribution. This distribution
    allocates uniform probability mass to as yet unseen events by using the
    number of events that have only been seen once. The probability mass
    ...
    def __init__( self, freqdist, bins = None, **kwargs ):
        ...
        self._totalMapTypes = _fertilityData[ 0 ]
        self._fertility = _fertilityData[ 1 ][ kwargs.get( 'state' ) ]
        self._weight = globalModelParameters.modWB_weight
    ...
    def prob( self, sample ):
        ...
        c = self._freqdist[ sample ]
        if c:
            return self._weight * (c / float( self._N + self._T )) + \
                   (1 - self._weight) * self._fertility[ sample ] / self._totalMapTypes
        else:
            return self._P0
...
class MutableProbDist( ProbDistI ):
    ...
    def __init__( self, prob_dist, samples, store_logs = True, **kwargs ):
        """
...
class KneserNeyProbDist( ProbDistI ):
    ...
    def __init__( self, freqdist, bins = None, discount = 0.75, **kwargs ):
        """
...
class ConditionalFreqDist( defaultdict ):
    ...
    def plot( self, *args, **kwargs ):
        ...
        samples = _get_kwarg( kwargs, 'samples',
                              sorted( set(
                                  v for c in conditions for v in self[ c ] ) ) )  # this computation could be wasted
        if not "linewidth" in kwargs:
            kwargs[ "linewidth" ] = 2
...
class ConditionalProbDist( ConditionalProbDistI ):
    ...
    def __init__( self, cfdist, probdist_factory,
        ...
        for condition in cfdist:
            factory_kw_args[ 'state' ] = condition
